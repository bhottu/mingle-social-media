    ...
    <div class="p-4">
        <div class="flex items-center justify-between">
            ...
            @if(Auth::id() === $share->user_id)
                <div class="relative" x-data="{ isOpen: false }">
                    ...
                    <form action="{{ route('profile.posts.destroy', $share->id) }}" class="block px-4 py-2 text-red-600 hover:bg-gray-200" method="POST">
                        @csrf
                        @method('DELETE')
                        <button type="submit" onclick="return confirm('Are you sure?')">Delete </button>
                    </form>
        ...
        <div class="flex justify-between items-center mt-4">
            ...
            <div class="flex items-center space-x-2">
                ...
                <form action="{{ route('post.share', ['post' => $share->post->id]) }}" method="POST">
                    @csrf
                    <button type="submit" class="flex items-center space-x-1">
                        <i class="fas fa-share-alt"></i>
                        <span class="ml-1">Share</span>
